...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        gpt4o_latency = f"{result['gpt4o_latency']:.3f}"
        mini_latency = f"{result['mini_latency']:.3f}"
        line = f"{prompt:<40} | {gpt4o_response:<40} | {mini_response:<40} | {gpt4o_latency:<15} | {mini_latency:<15}"
        lines.append(line)
    return "\n".join(lines)
